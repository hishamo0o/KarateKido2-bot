...
1.  **Masks the Tree**: Isolates the trunk based on its calibrated color.
2.  **Scans for Branches**: Uses horizontal line detection on the left and right sides of each segment.
3.  **Identifies Hits**: If no branch is found, it scans the trunk for number indicators (2, 3, or 4) to determine how many times to hit.
4.  **Queues Moves**: All 7 segments are processed at once, and the resulting moves are queued for execution.
